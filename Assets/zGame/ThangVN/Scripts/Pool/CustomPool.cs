...
//{
    public class PoolsContainer<T> where T : Component
    {
        ...
        public void Add(CustomPool<T> pPool)
        {
            ...
            else
            {
                ...
                //Merge two pool
                int lengthF2 = pPool.ActiveList.Count;
                for (int k = 0; k < lengthF2; k++)
                {
                    var obj = pPool.ActiveList[k];
                    //foreach (var obj in pPool.ActiveList)
                    if (!pool.ActiveList.Contains(obj))
                        pool.ActiveList.Add(obj);
                }
                lengthF2 = pPool.InactiveList.Count;
                for (int k = 0; k < lengthF2; k++)
                {
                    var obj = pPool.InactiveList[k];
        ...
        {
            var list = new List<T>();
            var enumerator = poolDict.GetEnumerator();
    ...
    public class CustomPool<T> where T : Component
    {
        ...
        public CustomPool(T pPrefab, int pInitialCount, Transform pParent, bool pBuildinPrefab, string pName = "", bool pAutoRelocate = true)
        {
            ...
            if (pBuildinPrefab)
            {
                //mInactiveList.Add(mPrefab);//FIXME phai tat di vi` khi spawn ma` het , no tu. dong khoi? tao. obj clone tu 1 cai object dang hoat dong.
                //trong khi do co the cai obj do' dang co' nhung~ obj effect khac' dua vao` ben trong roi
